...
new_stocks = dax40 + eurostoxx50 + ftse100 + aex25 + ibex35 + smi20 + sp400

with open('/home/theo/Helios/src/lib/data/stock-universe.ts', 'r') as f:
    content = f.read()

match = re.search(r'export const DEFAULT_STOCKS: StockInfo\[\] = \[(.*?)\];', content, re.DOTALL)
old_stocks = []
if match:
    existing_str = match.group(1)
    for line in existing_str.split('\n'):
        if not line.strip(): continue
        sym_match = re.search(r'symbol: "(.*?)"', line)
        name_match = re.search(r'name: "(.*?)"', line)
        sector_match = re.search(r'sector: "(.*?)"', line)
        if sym_match and name_match and sector_match:
            sym = sym_match.group(1)
            market = "CAC40" if sym.endswith('.PA') else "SP500"
            old_stocks.append({
                "symbol": sym,
...
for s in all_stocks:
    name = s["name"].replace('"', '\\"')
    out += f'  {{ symbol: "{s["symbol"]}", name: "{name}", sector: "{s["sector"]}", market: "{s["market"]}" }},\n'
...
with open('/home/theo/Helios/src/lib/data/stock-universe.ts', 'w') as f:
    f.write(out)

print(f"Updated stock-universe.ts with {len(all_stocks)} stocks.")
